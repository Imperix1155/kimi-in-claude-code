...
- [ ] **KMP-20** Delete superseded codex engine files (`scripts/lib/app-server.mjs`, `scripts/app-server-broker.mjs`, `scripts/lib/app-server-protocol.d.ts`, `scripts/lib/codex.mjs`) once the companion CLI rewrite (KMP-7) stops importing them; adapt `session-lifecycle-hook.mjs` + `stop-review-gate-hook.mjs` to the new env names (`KIMI_COMPANION_BROKER_*`) at the same time. _(discovered during KMP-6)_
